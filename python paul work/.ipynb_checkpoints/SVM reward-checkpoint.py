...
type_of_analysis= "REWARD" ### choices are "REWARD ONLY". if GO or LICK then use other SVM script.
analyse_current = True ## True if dff data used to analyse current label.  False if used to analyse label of previous trial
#########################
...
for trial in range(all_trials.shape[0]):
    ...
    ## populate trial number - the trials should be in order, but we use this data to make sure of it
    trial_list[trial,4]=all_trials[trial].num_trial
    
    ## populate dff data
    trial_dff[trial,0:41]=all_trials[trial].dff 

    ## dont thinkthis is used 
...
print("Before restricting to just licks")
...
##################################
##### new for the reward analysis
### only keep the lick trials
### (we are taking the lick trials and trying to decide if we can separate go and nogo)
mask= (trial_list[:,3]==0)

all_trials=np.ma.masked_array(all_trials)
all_trials[mask]=np.ma.masked
all_trials=all_trials.compressed()

trial_dff=np.ma.masked_array(trial_dff)
trial_dff[mask]=np.ma.masked
trial_dff=np.ma.compress_rowcols(trial_dff,axis=0)

trial_licks=np.ma.masked_array(trial_licks)
trial_licks[mask]=np.ma.masked
trial_licks=np.ma.compress_rowcols(trial_licks,axis=0)

trial_list=np.ma.masked_array(trial_list)
trial_list[mask]=np.ma.masked
trial_list=np.ma.compress_rowcols(trial_list,axis=0)
##########################


print("\nAfter restricting to just licks")
print("Array of neuron_num and trial type",trial_list.shape)
print("Array of dff",trial_dff.shape)
neuron_list=np.unique(trial_list[:,0]).reshape(-1,1)
print("Num of neurons before filter, after restricting to LICK trials=",neuron_list.shape[0])
...
for neuron in neuron_list:
    ...
    ######################################
    #### exlcude the first 8 data dff points of the trial (or ome other cut down)
    if type_of_analysis == "REWARD":
        neuron_data_dff = neuron_data_dff[:,28:40]
    ######################################
    
    if analyse_current==True:
        #######################
        ##### either just analyse dff predicts current label 
        X=neuron_data_dff.copy()
        y=neuron_data_list[:,1]
    elif analyse_current==False:
        #####################
        #### or analyse dff predicts previous label
        ## having analysed the data I see that it always is in correct order 
        ### so i have not implemented method to sort the trials
        ### simply shift by one
        X=neuron_data_dff[1:,:].copy()
        y=neuron_data_list[:-1,1]
        ##################
    
    ratio_size=.01
    min_number=5 # min number of trials required after cut down applied.
    ...
    if (y == 0).sum()/y.shape[0]>=ratio_size and (y == 1).sum()/y.shape[0]>=ratio_size and y.shape[0]>min_number:
        ...
        #pca = PCA(whiten=whitening, n_components=10)  
        pca.fit(X)
        ...
        ranks.append(-100)
        print("Excluded. Number in each category: No go",(y == 0).sum(),"Yes go ", (y == 1).sum())
        count+=1
...
### alternative files saved for excluding the first 8 data points from the time series
np.save("SVM_outputs/"+brain_area+"_"+type_of_analysis+"_experiments28to39.npy",experiments)
np.save("SVM_outputs/"+brain_area+"_"+type_of_analysis+"_accuracies28to39.npy",accuracies)
np.save("SVM_outputs/"+brain_area+"_"+type_of_analysis+"_ranks28to39.npy",ranks)
np.save("SVM_outputs/"+brain_area+"_"+type_of_analysis+"_dummy_distn28to39.npy",dummy_distn)
np.save("SVM_outputs/"+brain_area+"_"+type_of_analysis+"_Cs28to39.npy",Cs)
np.save("SVM_outputs/"+brain_area+"_"+type_of_analysis+"_mouse_ids28to39.npy",mouse_ids)

...
num_neurons=2 # number of neurons ti pass simultaneously into the SVM. has to be 2 athe momnet
#########################
...
rank_matrix=np.zeros((len(neurons_to_rank),len(allowed_additional_neurons)))
...
for neuron in neuron_list:
    ...
    # #####EITHER
    # ## combine neuron with each of the allowed "2nd neurons"    
    # if use_neuron:    
    #     for j in range(len(allowed_additional_neurons)):
    ...
    #             neuron_extra=allowed_additional_neurons[j]        
    ...
    #####OR
    if use_neuron:
        ## just combine all the neuronsin the neurons_to_rank into one X.  
        ## do this for each time we have use_neuron - so we get the same thing mulitple times for stats purposes. 
        for j in range(len(allowed_additional_neurons)):
            print("\n",count, str(neuron_list[count,0]), "XXXXXXXXXXXXXXXXXXXX")
            neuron_extra=allowed_additional_neurons[j] 
            neuron_data_list= trial_list[trial_list[:,0]==neuron_extra].copy()
            neuron_data_dff= trial_dff[trial_list[:,0]==neuron_extra].copy()  
            if j==0:
                X=neuron_data_dff.copy()
                y=neuron_data_list[:,1]
            else:
                X_extra=neuron_data_dff.copy()
                y_extra=neuron_data_list[:,1]
                assert y_extra.all() == y.all(), "Not all neurons for the SVM have been taken from the same experiment"
                X=np.append(X,X_extra, axis=1)            
            #####################################          
        ...
        if use_neuron and (y == 0).sum()/y.shape[0]>=ratio_size and (y == 1).sum()/y.shape[0]>=ratio_size and y.shape[0]>min_number:
            ...
            pca = PCA(whiten=whitening, n_components=5)  
            pca.fit(X)
        ...
        rank_matrix[neuron_count,j]=my_rank
